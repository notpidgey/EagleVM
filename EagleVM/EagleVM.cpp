...
int main(int argc, char* argv[])
{
	...
	std::printf("%2s %-10s %-10s %-10s\n", "", "name", "va", "size");
	std::for_each(sections.begin(), sections.end(), [&i](PIMAGE_SECTION_HEADER image_section)
		{
			std::printf("%2i %-10s %-10d %-10d\n", i, image_section->Name, image_section->VirtualAddress, image_section->SizeOfRawData);
			i++;
		});
	...
	std::printf("\n[>] %i imports\n", imports.size());
	std::printf("%3s %-20s %-s\n", "", "source", "import");
	std::for_each(imports.begin(), imports.end(), [&i](std::pair<std::string, std::string>& dll_import)
		{
	...
	auto sections = generator.build_section_table(sections);
