...
handle_info vm_handle_generator::create_vm_exit()
{
    ...
    //pushfq
    {
        std::vector<uint8_t> encoded_instruction = zydis_helper::encode_instruction_0(ZYDIS_MNEMONIC_POPFQ);
    ...
    for (int i = ZYDIS_REGISTER_R15; i >= ZYDIS_REGISTER_RAX; i--)
    {
        ...
        std::vector<uint8_t> encoded_instruction = zydis_helper::encode_instruction_1(ZYDIS_MNEMONIC_POP, reg);
        vm_exit_operations.push_back(encoded_instruction);
    }
