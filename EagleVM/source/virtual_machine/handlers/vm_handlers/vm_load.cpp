...
void vm_load_handler::construct_single(function_container& container, reg_size reg_size)
{
    ...
    if (reg_size == reg_size::bit64)
    {
        ...
        // call push
        container.add(zydis_helper::encode<ZYDIS_MNEMONIC_MOV, zydis_ereg, zydis_emem>(ZREG(VTEMP), ZMEMBI(VREGS, VTEMP, 1, reg_size)));
        call_vm_handler(container, push_handler->get_handler_va(reg_size));
    }
    else if (reg_size == reg_size::bit32)
    {
        container.add(zydis_helper::encode<ZYDIS_MNEMONIC_MOV, zydis_ereg, zydis_emem>(ZREG(TO32(VTEMP)), ZMEMBI(VREGS, VTEMP, 1, reg_size)));
        call_vm_handler(container, push_handler->get_handler_va(reg_size));
    }
    else if (reg_size == reg_size::bit16)
    {
        container.add(zydis_helper::encode<ZYDIS_MNEMONIC_MOV, zydis_ereg, zydis_emem>(ZREG(TO16(VTEMP)), ZMEMBI(VREGS, VTEMP, 1, reg_size)));
        call_vm_handler(container, push_handler->get_handler_va(reg_size));
    }
