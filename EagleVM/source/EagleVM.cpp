...
int main(int argc, char *argv[]) {
  ...
  for (int c = 0; c < vm_iat_calls.size(); c += 2) // i1 = vm_begin, i2 = vm_end
  {
    ...
    codec::decode_vec instructions =
        codec::get_instructions(pinst_begin, pinst_end - pinst_begin);
    dasm::segment_dasm_ptr dasm = std::make_shared<dasm::segment_dasm>(
        instructions, rva_inst_begin, rva_inst_end);
  ...
  // custom entry point
  {
    ...
    codec::encoded_vec packer_code_bytes =
        packer_sm.compile_section(packer_section.virtual_address);
