...
namespace eagle::ir::handler
{
    ...
    ir_insts x_or::gen_handler(handler_sig signature)
    {
        ...
        // in x_orition, it gives room for mapping x86 context into random places as well
        ...
        return {
            ...
            make_dyn(codec::m_xor, encoder::reg(vtemp2), encoder::reg(vtemp)),
...
{
    translate_mem_result x_or::translate_mem_action(const codec::dec::op_mem& op_mem, uint8_t idx)
    {
        return idx == 0 ? translate_mem_result::both : base_x86_translator::translate_mem_action(op_mem, idx);
    }

    translate_status x_or::encode_operand(codec::dec::op_imm op_imm, uint8_t)
    {
    ...
    void x_or::finalize_translate_to_virtual(x86_cpu_flag flags)
    {
