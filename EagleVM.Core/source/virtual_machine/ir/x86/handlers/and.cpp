...
namespace eagle::ir::handler
{
    ...
    ir_insts and ::gen_handler(handler_sig signature)
    {
        ...
        // in andition, it gives room for mapping x86 context into random places as well
        ...
        return { std::make_shared<cmd_pop>(vtemp, target_size), std::make_shared<cmd_pop>(vtemp2, target_size),
                 std::make_shared<cmd_x86_dynamic>(codec::m_and, vtemp2, vtemp), std::make_shared<cmd_push>(vtemp2, target_size) };
    }
} // namespace eagle::ir::handler

namespace eagle::ir::lifter
{
    translate_mem_result and ::translate_mem_action(const codec::dec::op_mem &op_mem, uint8_t idx)
    {
        return idx == 0 ? translate_mem_result::both : base_x86_translator::translate_mem_action(op_mem, idx);
    }

    translate_status and ::encode_operand(codec::dec::op_imm op_imm, uint8_t)
    {
    ...
    void and ::finalize_translate_to_virtual(x86_cpu_flag flags)
    {
