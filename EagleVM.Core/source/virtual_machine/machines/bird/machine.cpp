...
namespace eagle::virt::eg
{
    ...
    void bird_machine::handle_cmd(const asmb::code_container_ptr& block, const ir::cmd_branch_ptr& cmd)
    {
        ...
        if (cmd->is_virtual())
        {
            create_handler(true, [&](const encode_builder_ptr& out, const std::function<reg()>& alloc_reg)
            {
                // push all
    ...
    void bird_machine::handle_cmd(const asmb::code_container_ptr& block, const ir::cmd_mem_read_ptr& cmd)
    {
        ...
        create_handler(false, [&](const encode_builder_ptr& out, const std::function<reg()>& alloc_reg)
        {
    ...
    {
        handle_generic_logic_cmd(m_and, cmd->get_size(), cmd->get_preserved());
    }

    void bird_machine::handle_cmd(const asmb::code_container_ptr& block, const ir::cmd_or_ptr& cmd)
    {
        handle_generic_logic_cmd(m_or, cmd->get_size(), cmd->get_preserved());
    }

    void bird_machine::handle_cmd(const asmb::code_container_ptr& block, const ir::cmd_xor_ptr& cmd)
    {
        handle_generic_logic_cmd(m_xor, cmd->get_size(), cmd->get_preserved());
    }
    ...
    {
        handle_generic_logic_cmd(m_add, cmd->get_size(), cmd->get_preserved());
    }

    void bird_machine::handle_cmd(const asmb::code_container_ptr& block, const ir::cmd_sub_ptr& cmd)
    {
        handle_generic_logic_cmd(m_sub, cmd->get_size(), cmd->get_preserved());
    }
    ...
    void bird_machine::handle_cmd(const asmb::code_container_ptr& block, const ir::cmd_cnt_ptr& cmd)
    {
        ...
        create_handler(false, [&](const encode_builder_ptr& out, const std::function<reg()>& alloc_reg)
        {
            ...
            if (is_bit8)
            {
                ...
                block->add(encode(m_xor, ZREG(pop_reg_size), ZREG(pop_reg_size)));
    ...
    void bird_machine::handle_generic_logic_cmd(const mnemonic command, const ir::ir_size size, const bool preserved)
    {
    ...
    void bird_machine::create_handler(bool force_inline, std::function<void(encode_builder_ptr&, std::function<reg()>)> handler_create)
    {
        scope_register_manager scope = reg_64_container->create_scope();
