...
namespace eagle::virt
{
    ...
    std::vector<asmb::function_container> vm_virtualizer::virtualize_segment(dasm::segment_dasm* dasm)
    {
        for (dasm::basic_block* block : dasm->blocks)
        {
            std::string block_name = "block:" + std::to_string(block->start_rva);
            block_labels_[block] = asmb::code_label::create(block_name);
        }

        std::vector<asmb::function_container> virtualized_blocks;
        for (dasm::basic_block* block : dasm->blocks)
        {
            asmb::function_container block_container = virtualize_block(dasm, block);
    ...
    asmb::function_container vm_virtualizer::virtualize_block(dasm::segment_dasm* dasm, dasm::basic_block* block)
    {
        ...
        for (int i = 0; i < block->decoded_insts.size() + skip; i++)
        {
            ...
            if (virt_status)
            {
                ...
                if (!current_vm)
                {
                    asmb::code_label* vmenter_return_label = asmb::code_label::create("vmenter_return:" + current_rva);
            ...
            else
            {
                ...
                if (current_vm)
                {
                    asmb::code_label* jump_label = asmb::code_label::create("vmleave_dest:" + current_rva);
        ...
        if (current_vm)
        {
            asmb::code_label* jump_label = asmb::code_label::create("vmexit dest: " + current_rva, true);
            call_vm_exit(container, jump_label);
        ...
        switch (end_reason)
        {
            case dasm::block_end:
            {
                ...
                asmb::code_label* jump_label = nullptr;

                auto [target, type] = dasm->get_jump(block);
                if (type == dasm::jump_outside_segment)
                {
                    // we return back to .text
                    jump_label = asmb::code_label::create("vmleave_dest:" + target, true);
            ...
            case dasm::block_jump:
            {
                ...
                if (type == dasm::jump_outside_segment)
                {
                    ...
                    asmb::code_label* jump_label = asmb::code_label::create("vmleave_dest:" + target);
                    jump_label->finalize(target);
    ...
    asmb::code_label* vm_virtualizer::get_label(dasm::basic_block* block)
    {
    ...
    void vm_virtualizer::call_vm_enter(asmb::function_container& container, asmb::code_label* target)
    {
        ...
        asmb::code_label* rel_label = asmb::code_label::create("call_vm_enter_rel");
        container.add(rel_label, RECOMPILE(zydis_helper::enc(ZYDIS_MNEMONIC_JMP, ZJMP(vmenter_address, rel_label))));
    }

    void vm_virtualizer::call_vm_exit(asmb::function_container& container, asmb::code_label* target)
    {
    ...
    void vm_virtualizer::create_vm_jump(zyids_mnemonic mnemonic, asmb::function_container& container, asmb::code_label* rva_target)
    {
        asmb::code_label* rel_label = asmb::code_label::create("call_vm_enter_rel");
        container.add(rel_label, RECOMPILE(zydis_helper::enc(mnemonic, ZJMP(rva_target, rel_label))));
    }

    std::pair<bool, asmb::function_container> vm_virtualizer::translate_to_virtual(const zydis_decode& decoded_instruction,
    ...
    encoded_vec vm_virtualizer::create_jump(const uint32_t rva, asmb::code_label* rva_target)
    {
