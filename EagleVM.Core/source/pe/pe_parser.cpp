...
namespace eagle::pe
{
    ...
    bool pe_parser::read_file()
    {
        ...
        bool success = !!ReadFile(file, unprotected_pe_.data(), (DWORD)unprotected_pe_.size(), &bytes_read, nullptr);
        CloseHandle(file);
    ...
    std::vector<std::pair<uint32_t, stub_import>> pe_parser::find_iat_calls()
    {
        ...
        enum_imports(
            [&stub_dll_imports](const PIMAGE_IMPORT_DESCRIPTOR import_descriptor, const PIMAGE_THUNK_DATA thunk_data,
            const PIMAGE_SECTION_HEADER import_section,
            int index, uint8_t* data_base)
            {
                ...
                if (std::strcmp((char*)import_library, "EagleVMStub.dll") == 0)
                {
                    ...
                    IMAGE_IMPORT_BY_NAME* import_by_name = (IMAGE_IMPORT_BY_NAME*)(import_section_raw + (thunk_data->u1.AddressOfData - import_section
                        ->VirtualAddress));
    ...
    void pe_parser::enum_imports(
        const std::function<void(PIMAGE_IMPORT_DESCRIPTOR, PIMAGE_THUNK_DATA, PIMAGE_SECTION_HEADER, int index, uint8_t*)> import_proc)
    {
        ...
        PIMAGE_IMPORT_DESCRIPTOR import_descriptor = reinterpret_cast<PIMAGE_IMPORT_DESCRIPTOR>(import_data + (image_nt_headers->OptionalHeader.
            DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].VirtualAddress - import_section->VirtualAddress));
        for (; import_descriptor->Name != 0; import_descriptor++)
        {
            ...
            for (PIMAGE_THUNK_DATA thunk_data = reinterpret_cast<PIMAGE_THUNK_DATA>(import_data + (thunk - import_section->VirtualAddress));
                 thunk_data->u1.AddressOfData != 0; thunk_data++)
            {
